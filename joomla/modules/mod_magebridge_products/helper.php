...
class ModMageBridgeProductsHelper extends MageBridgeModuleHelper
{
	...
	{
		$register = array(array('api', 'magebridge_product.list', self::getArguments($params)),);
	...
	{
		$products = parent::getCall('getAPI', 'magebridge_product.list', self::getArguments($params));
		...
		if (!empty($products))
		{
			foreach ($products as $index => $product)
			{
				...
				if ($params->get('override_product_url', 0) == 1)
				{
					$product['url'] = null;
				}
