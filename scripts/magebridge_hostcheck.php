...
class systemCheck
{
    ...
    private function sessionSavePath()
    {
        ...
        if(empty($path)) {
            ...
            return array($path . ' is not readable', false);
    ...
    private function isJoomla()
    {
        ...
        define('_JEXEC', 1);
        $version_file = JPATH_BASE.'/libraries/joomla/version.php';
        if(!is_file($version_file)) $version_file = JPATH_BASE.'/libraries/cms/version/version.php';
        if(!is_file($version_file)) return array('Not found', false);
    ...
    private function version($package = 'php')
    {
        ...
        switch($package) {
            ...
            case 'apache':
                ...
                if(isset($match[1])) $current = $match[1];
                $minimum = '2.0.61';
                break;
        }
    ...
    {
        $user = self::runCommand('whoami');
        if(empty($user)) $user = getmyuid();
        if(empty($user)) return array('Not found', false);
        return array($user, true);
    }

    private function checkCURL()
    {
        $url = 'http://lang.magebridge.com/magebridge.php';
