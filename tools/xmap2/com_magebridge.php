...
class xmap_com_magebridge {
    ...
    public function getTree( &$xmap, &$parent, &$params ) 
    {
        ...
        if(preg_match('/index.php\?option=com_magebridge\&view=catalog\&layout=category\&request=([0-9]+)/', $parent->link, $match)) {
            if((!isset($match[1]) || !is_numeric($match[1])) && isset($params['expand_category']) && $params['expand_category'] == 0) {
                return;
            }

            // Set the current category ID to the ID configured within the Menu-Item
            $category = $match[1];
        }
        ...
        // Set the current category ID to the Root Catalog if needed
